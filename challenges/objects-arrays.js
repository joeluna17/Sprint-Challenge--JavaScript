...
const contactInfo = [];
for(let i=0; i<graduates.length; i++) {
  const predicate = graduates[i].first_name + " " + graduates[i].email
  contactInfo.push(predicate)
}
...
const uni = [];
for(let i=0; i<graduates.length; i++){
  let result = graduates[i].university.includes("Uni")
  if (result){
...
populationTotal = zooAnimals.reduce((accum, currVal) => {
  return accum += currVal.population
